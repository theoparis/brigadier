...
public class SuggestionsBuilderTest {
	...
	public void suggest_appends() {
		...
		assertThat(
				result.getList(),
				equalTo(Lists.newArrayList(new Suggestion(StringRange.between(6, 7), "world!"))));
	...
	public void suggest_multiple() {
		final Suggestions result =
				builder.suggest("world!").suggest("everybody").suggest("weekend").build();
		assertThat(
				result.getList(),
				equalTo(
						Lists.newArrayList(
	...
	public void sort_alphabetical() {
		Suggestions result =
				builder.suggest("2")
						...
						.build();
		List<String> actual =
				result.getList().stream().map(Suggestion::getText).collect(Collectors.toList());
	...
	public void sort_numerical() {
		Suggestions result =
				builder.suggest(2).suggest(4).suggest(6).suggest(8).suggest(30).suggest(32).build();
		List<String> actual =
				result.getList().stream().map(Suggestion::getText).collect(Collectors.toList());
	...
	public void sort_mixed() {
		...
		assertThat(
				...
				equalTo(
						Lists.newArrayList(
								"11", "2", "22", "33", "3a", "4", "6", "8", "30", "32", "a", "a3",
								"b", "c")));
	}
}
