...
public class CommandDispatcherTest {
	...
	@Mock private Command<Object> command;
	@Mock private Object source;
	@Mock private ResultConsumer<Object> consumer;
	...
	public void testExecuteUnknownCommand() {
		...
		try {
			...
			assertThat(
					ex.getType(),
					is(CommandSyntaxException.BUILT_IN_EXCEPTIONS.dispatcherUnknownCommand()));
	...
	public void testExecuteUnknownSubcommand() {
		...
		try {
			...
			assertThat(
					ex.getType(),
					is(CommandSyntaxException.BUILT_IN_EXCEPTIONS.dispatcherUnknownArgument()));
	...
	public void testExecuteRedirectedMultipleTimes() throws Exception {
		final LiteralCommandNode<Object> concreteNode =
				subject.register(literal("actual").executes(command));
		final LiteralCommandNode<Object> redirectNode =
		...
		assertThat(
				parse.getContext().getNodes().get(0).getRange(),
				equalTo(parse.getContext().getRange()));
	...
	public void testCorrectExecuteContextAfterRedirect() throws Exception {
		...
		subject.register(
				add.then(addArg.redirect(root, c -> c.getSource() + getInteger(c, "value"))));
	...
	public void testSharedRedirectAndExecuteNodes() throws CommandSyntaxException {
		...
		subject.register(
				add.then(
						addArg.redirect(root, c -> c.getSource() + getInteger(c, "value"))
	...
	public void testExecuteRedirected() throws Exception {
		...
		when(modifier.apply(argThat(hasProperty("source", is(source)))))
	...
	public void testIncompleteRedirectShouldThrow() {
		final LiteralCommandNode<Object> foo =
				subject.register(
						literal("foo")
								.then(
										literal("bar")
	...
	public void testExecuteInvalidSubcommand() {
		...
		try {
			...
			assertThat(
					ex.getType(),
					is(CommandSyntaxException.BUILT_IN_EXCEPTIONS.readerExpectedInt()));
			assertThat(ex.getCursor(), is(4));
		}
	}
	...
	public void testResultConsumerInForkedNonErrorRun() throws CommandSyntaxException {
		...
		subject.register(
				literal("repeat").fork(subject.getRoot(), context -> Arrays.asList(contexts)));

		assertThat(subject.execute("repeat foo", source), is(contexts.length));
		verify(consumer)
				.onCommandComplete(argThat(contextSourceMatches(contexts[0])), eq(true), eq(9));
		verify(consumer)
				.onCommandComplete(argThat(contextSourceMatches(contexts[1])), eq(true), eq(10));
		verify(consumer)
				.onCommandComplete(argThat(contextSourceMatches(contexts[2])), eq(true), eq(11));
		verifyNoMoreInteractions(consumer);
	}
	...
	public void testExceptionInNonForkedCommand() throws CommandSyntaxException {
		...
		final CommandSyntaxException exception =
				CommandSyntaxException.BUILT_IN_EXCEPTIONS.readerExpectedBool().create();
	...
	public void testPartialExceptionInForkedRedirect() throws CommandSyntaxException {
		...
		subject.register(
				literal("split")
						.fork(
								subject.getRoot(),
								context -> Arrays.asList(source, rejectedSource, otherSource)));
		subject.register(
				literal("filter")
						.fork(
								subject.getRoot(),
		...
		verify(consumer)
				.onCommandComplete(argThat(contextSourceMatches(rejectedSource)), eq(false), eq(0));
		verify(consumer).onCommandComplete(argThat(contextSourceMatches(source)), eq(true), eq(3));
		verify(consumer)
				.onCommandComplete(argThat(contextSourceMatches(otherSource)), eq(true), eq(3));
		verifyNoMoreInteractions(consumer);
	}
	...
	public static Matcher<CommandContext<Object>> contextSourceMatches(final Object source) {
		return new CustomMatcher<CommandContext<Object>>("source " + source) {
			...
			public boolean matches(Object object) {
				return (object instanceof CommandContext)
						&& ((CommandContext<?>) object).getSource() == source;
			}
		};
	}
}
