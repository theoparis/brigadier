...
public class LiteralCommandNodeTest extends AbstractCommandNodeTest {
	...
	public void setUp() throws Exception {
		...
		contextBuilder =
				new CommandContextBuilder<>(
						new CommandDispatcher<>(), new Object(), new RootCommandNode<>(), 0);
	}
	...
	public void testParseSimilar() throws Exception {
		...
		try {
			...
			assertThat(
					ex.getType(),
					is(CommandSyntaxException.BUILT_IN_EXCEPTIONS.literalIncorrect()));
	...
	public void testSuggestions() throws Exception {
		final Suggestions empty =
				node.listSuggestions(contextBuilder.build(""), new SuggestionsBuilder("", 0))
						.join();
		assertThat(
				empty.getList(),
				equalTo(Lists.newArrayList(new Suggestion(StringRange.at(0), "foo"))));

		final Suggestions foo =
				node.listSuggestions(contextBuilder.build("foo"), new SuggestionsBuilder("foo", 0))
						.join();
		assertThat(foo.isEmpty(), is(true));

		final Suggestions food =
				node.listSuggestions(
								contextBuilder.build("food"), new SuggestionsBuilder("food", 0))
						.join();
		assertThat(food.isEmpty(), is(true));

		final Suggestions b =
				node.listSuggestions(contextBuilder.build("b"), new SuggestionsBuilder("b", 0))
						.join();
		assertThat(food.isEmpty(), is(true));
	}
	...
	public void testEquals() throws Exception {
		...
		new EqualsTester()
				.addEqualityGroup(literal("foo").build(), literal("foo").build())
				.addEqualityGroup(
						literal("bar").executes(command).build(),
						literal("bar").executes(command).build())
				.addEqualityGroup(literal("bar").build(), literal("bar").build())
				.addEqualityGroup(
