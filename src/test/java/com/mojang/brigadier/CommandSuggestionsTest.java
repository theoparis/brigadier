...
public class CommandSuggestionsTest {
	...
	private void testSuggestions(
		...
		final Suggestions result =
				subject.getCompletionSuggestions(subject.parse(contents, source), cursor).join();
	...
	public void getCompletionSuggestions_rootCommands() throws Exception {
		...
		final Suggestions result =
				subject.getCompletionSuggestions(subject.parse("", source)).join();
	...
	public void getCompletionSuggestions_rootCommands_withInputOffset() throws Exception {
		...
		final Suggestions result =
				subject.getCompletionSuggestions(subject.parse(inputWithOffset("OOO", 3), source))
						.join();
	...
	public void getCompletionSuggestions_rootCommands_partial() throws Exception {
		...
		final Suggestions result =
				subject.getCompletionSuggestions(subject.parse("b", source)).join();
	...
	public void getCompletionSuggestions_rootCommands_partial_withInputOffset() throws Exception {
		...
		final Suggestions result =
				subject.getCompletionSuggestions(subject.parse(inputWithOffset("Zb", 1), source))
						.join();
	...
	public void getCompletionSuggestions_subCommands() throws Exception {
		subject.register(
				literal("parent").then(literal("foo")).then(literal("bar")).then(literal("baz")));

		final Suggestions result =
				subject.getCompletionSuggestions(subject.parse("parent ", source)).join();
	...
	public void getCompletionSuggestions_movingCursor_subCommands() throws Exception {
		...
		testSuggestions(
				"parent_one faz ", 1, StringRange.between(0, 1), "parent_one", "parent_two");
		testSuggestions(
				"parent_one faz ", 7, StringRange.between(0, 7), "parent_one", "parent_two");
	...
	public void getCompletionSuggestions_subCommands_partial_withInputOffset() throws Exception {
		...
		final ParseResults<Object> parse =
				subject.parse(inputWithOffset("junk parent b", 5), source);
		final Suggestions result = subject.getCompletionSuggestions(parse).join();
	...
	public void getCompletionSuggestions_redirect() throws Exception {
		final LiteralCommandNode<Object> actual =
				subject.register(literal("actual").then(literal("sub")));
		...
		assertThat(
				result.getList(),
				equalTo(Lists.newArrayList(new Suggestion(StringRange.at(9), "sub"))));
	}
	...
	public void getCompletionSuggestions_redirectPartial() throws Exception {
		...
		assertThat(
				result.getList(),
				equalTo(Lists.newArrayList(new Suggestion(StringRange.between(9, 10), "sub"))));
	}

	@Test
	public void getCompletionSuggestions_movingCursor_redirect() throws Exception {
		final LiteralCommandNode<Object> actualOne =
				subject.register(
						literal("actual_one")
								.then(literal("faz"))
								.then(literal("fbz"))
		...
		testSuggestions(
				...
				"redirect_two");
		testSuggestions(
				"redirect_one faz ", 9, StringRange.between(0, 9), "redirect_one", "redirect_two");
	...
	public void getCompletionSuggestions_redirectPartial_withInputOffset() throws Exception {
		...
		assertThat(
				result.getList(),
				equalTo(Lists.newArrayList(new Suggestion(StringRange.between(10, 11), "sub"))));
	}
	...
	public void getCompletionSuggestions_redirect_lots() throws Exception {
		...
		subject.register(
				literal("redirect")
						.then(literal("loop").then(argument("loop", integer()).redirect(loop))));

		final Suggestions result =
				subject.getCompletionSuggestions(
		...
		assertThat(
				result.getList(),
				equalTo(Lists.newArrayList(new Suggestion(StringRange.at(33), "loop"))));
	}
