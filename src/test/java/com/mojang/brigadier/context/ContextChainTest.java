...
public class ContextChainTest {
	...
	public void testExecuteAllForSingleCommand() throws CommandSyntaxException {
		...
		final ContextChain<Object> chain =
		...
		verify(consumer)
				.onCommandComplete(
						argThat(CommandDispatcherTest.contextSourceMatches(runtimeSource)),
						eq(true),
						eq(4));
	...
	public void testExecuteAllForRedirectedCommand() throws CommandSyntaxException {
		...
		dispatcher.register(literal("foo").executes(command));
		dispatcher.register(
				literal("bar").redirect(dispatcher.getRoot(), context -> redirectedSource));
		final Object source = "compile_source";
		...
		verify(consumer)
				.onCommandComplete(
						argThat(CommandDispatcherTest.contextSourceMatches(redirectedSource)),
						eq(true),
						eq(4));
		verifyNoMoreInteractions(consumer);
	}
	...
	public void testMultiStageExecution() {
		...
		final ContextChain<Object> stage0 =
				ContextChain.tryFlatten(topContext).orElseThrow(AssertionError::new);
