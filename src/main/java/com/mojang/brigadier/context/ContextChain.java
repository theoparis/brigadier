...
public class ContextChain<S> {
	// TODO ideally those two would have separate types, but modifiers and executables expect full
	// context
	...
	public ContextChain(
			final List<CommandContext<S>> modifiers, final CommandContext<S> executable) {
	...
	public static <S> Collection<S> runModifier(
			final CommandContext<S> modifier,
			final S source,
			final ResultConsumer<S> resultConsumer,
			final boolean forkedMode)
			throws CommandSyntaxException {
		final RedirectModifier<S> sourceModifier = modifier.getRedirectModifier();

		// Note: source currently in context is irrelevant at this point, since we might have
		// updated it in one of earlier stages
	...
	public static <S> int runExecutable(
			final CommandContext<S> executable,
			final S source,
			final ResultConsumer<S> resultConsumer,
			final boolean forkedMode)
			throws CommandSyntaxException {
	...
	public int executeAll(final S source, final ResultConsumer<S> resultConsumer)
			throws CommandSyntaxException {
