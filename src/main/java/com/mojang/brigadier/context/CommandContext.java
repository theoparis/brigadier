...
public class CommandContext<S> {
	...
	public CommandContext<S> copyFor(final S source) {
		...
		return new CommandContext<>(
				source, input, arguments, command, rootNode, nodes, range, child, modifier, forks);
	}
	...
	public <V> boolean hasArgumentOfType(final String name, final Class<V> clazz) {
		...
		return argument != null
				&& PRIMITIVE_TO_WRAPPER
						...
						.isAssignableFrom(argument.getResult().getClass());
	}
	...
	@SuppressWarnings("unchecked")
	public <V> V getArgumentOrDefault(
			final String name, final Class<V> clazz, final V defaultValue) {
		final ParsedArgument<S, ?> argument = arguments.get(name);
	...
	@SuppressWarnings("unchecked")
	public <V> V getArgumentOrCompute(
			final String name, final Class<V> clazz, final Supplier<V> defaultSupplier) {
		final ParsedArgument<S, ?> argument = arguments.get(name);
